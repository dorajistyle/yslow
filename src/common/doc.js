...
YSLOW.doc.addRuleInfo('_3po_asyncjs','Load third party javascript asynchronously','Always load third party javascript asynchronously. Third parties that will be checked are twitter, facebook, google (api, analythics, ajax), linkedin, disqus, pinterest & jquery.');
YSLOW.doc.addRuleInfo('_3po_jsonce','Load the third party javascript only once','Make sure the third party javascripts are loaded only once');
YSLOW.doc.addRuleInfo('cssprint','Avoid loading specific css for print','Loading a specific stylesheet for print, can block rendering in your browser (depending on version) and will for almost all browsers, block the onload event to fire (even though the print stylesheet is not even used).');
YSLOW.doc.addRuleInfo('cssinheaddomain','Load CSS in head from document domain','CSS files inside of head should be loaded from the same domain as the main document, in order to avoid DNS lookups. This is extra important for mobile.');
...
YSLOW.doc.addRuleInfo('totalrequests','Reduce number of total requests','Avoid have too many requests on your page. The more requests, the slower the page will be.');
YSLOW.doc.addRuleInfo('expiresmod','Have long expire headers for static components','This is the standard expires yslow rule with the twist that you do not get penalized using Google Analythics or Gaug.es.');
